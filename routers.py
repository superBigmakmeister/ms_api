...
# 1. Сначала маршрут без параметров (список всех экспонатов)
...
# 2. Затем все маршруты с конкретными строками (by-type, top-profit)
@router.get("/wings/by-type/{type_id}", tags=["🖼️ Экспонаты"])
def get_wings_by_type(type_id: int, db: Session = Depends(get_db)):
    """Получить все экспонаты указанного типа"""
    wings = db.query(models.Wing).filter(models.Wing.type_id == type_id).all()
    return create_response_with_sql(wings)

@router.get("/wings/top-profit", tags=["🖼️ Экспонаты"])
def get_top_profit_wings(
    min_profit: float = 1.5, 
    limit: int = 10, 
    db: Session = Depends(get_db)
):
    """Получить экспонаты с прибылью выше min_profit, отсортированные по убыванию прибыли"""
    wings = (db.query(models.Wing)
             .filter(models.Wing.profit > min_profit)
             .order_by(models.Wing.profit.desc())
             .limit(limit)
             .all())
    return create_response_with_sql(wings)

# 3. В самом конце — маршруты с динамическим параметром {wing_id}
...
@router.get("/types/statistics", tags=["📊 Аналитика"])
def get_type_statistics_endpoint(db: Session = Depends(get_db)):
    """Статистика по типам экспонатов: количество и процент от общего числа"""
    stats = crud.get_type_statistics(db)  # используем уже импортированный crud
